...
const SUGGESTED_QUESTIONS = {
    default: [
        { text: "Why is my bill high?", icon: "💸" },
        { text: "How can I save more?", icon: "💡" },
        { text: "What appliances use most power?", icon: "🔌" },
        { text: "Explain my forecast", icon: "📈" }
    ],
    dashboard: [
        { text: "Is my current usage normal?", icon: "⚡" },
        { text: "Why is power so high right now?", icon: "🔥" },
        { text: "When should I use heavy appliances?", icon: "⏰" }
    ],
    analytics: [
        { text: "What's my consumption trend?", icon: "📊" },
        { text: "Which hour costs me most?", icon: "💰" },
        { text: "Are there any anomalies?", icon: "⚠️" }
    ],
    control: [
        { text: "How to schedule my AC?", icon: "❄️" },
        { text: "Best time to run washing machine?", icon: "👕" },
        { text: "Optimize my device usage", icon: "🎯" }
    ]
};
    ...
    <div className="flex justify-start mb-3">
        <div className="bg-dashboard-textSecondary/10 rounded-2xl rounded-bl-sm px-4 py-3">
            <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-dashboard-textSecondary">SHEM is thinking...</span>
...
const SHEMChatWidget = ({
    ...
    const getSuggestedQuestions = () => {
        const contextQuestions = SUGGESTED_QUESTIONS[pageContext] || [];
        const defaultQuestions = SUGGESTED_QUESTIONS.default;
    ...
    return (
        <>
            ...
            >
                ...
                <span className="font-medium">Ask SHEM</span>
            </button>
                ...
                <div className="bg-dashboard-card border border-dashboard-textSecondary/20 rounded-2xl shadow-2xl overflow-hidden flex flex-col" style={{ height: '600px' }}>
                    ...
                    <div className="bg-gradient-to-r from-accent to-blue-600 px-4 py-3 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <h3 className="text-white font-bold text-sm">SHEM - Energy Advisor</h3>
                                <p className="text-white/70 text-xs">
                                    {rateLimitRemaining !== null && `${rateLimitRemaining} questions left`}
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    {showSettings && (
                        <div className="px-4 py-3 bg-dashboard-textSecondary/10 border-b border-dashboard-textSecondary/20">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-sm text-dashboard-text">Chat Settings</span>
                            ...
                            <p className="text-xs text-dashboard-textSecondary">
                                🔒 Your chat is processed securely. We don't store personal data.
                            </p>
                        </div>
                    )}
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                        {messages.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-center">
                                <SparklesIcon className="w-12 h-12 text-accent/50 mb-4" />
                                <h4 className="text-dashboard-text font-medium mb-2">Hi! I'm SHEM</h4>
                                <p className="text-dashboard-textSecondary text-sm mb-4">
                                    Your AI energy advisor. Ask me anything about your electricity usage!
                                </p>
                                {/* Suggested Questions */}
                    ...
                    {messages.length > 0 && !isLoading && messages[messages.length - 1]?.role === 'assistant' && (
                        <div className="px-4 py-2 border-t border-dashboard-textSecondary/10 flex gap-2 overflow-x-auto">
                            <button onClick={() => sendMessage("Tell me more")} className="flex-shrink-0 px-3 py-1.5 bg-dashboard-textSecondary/10 hover:bg-dashboard-textSecondary/20 rounded-full text-xs text-dashboard-text transition-colors">Tell me more</button>
                            <button onClick={() => sendMessage("How to implement this?")} className="flex-shrink-0 px-3 py-1.5 bg-dashboard-textSecondary/10 hover:bg-dashboard-textSecondary/20 rounded-full text-xs text-dashboard-text transition-colors">How to do this?</button>
                            <button onClick={() => sendMessage("What else can I do?")} className="flex-shrink-0 px-3 py-1.5 bg-dashboard-textSecondary/10 hover:bg-dashboard-textSecondary/20 rounded-full text-xs text-dashboard-text transition-colors">What else?</button>
                        </div>
                    )}
                    ...
                    <div className="p-4 border-t border-dashboard-textSecondary/20 bg-dashboard-card z-10">
                        <div className="flex items-center gap-2">
                            ...
                            <input
                                ...
                                placeholder={voiceStatus === 'listening' ? "Listening..." : "Ask about your energy usage..."}
